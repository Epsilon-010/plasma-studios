...
export const LiquidMetalButton = forwardRef<HTMLButtonElement, LiquidMetalButtonProps>(
    ({ children, icon, borderWidth = 4, metalConfig, size = "md", className, disabled, ...props }, ref) => {
        // Asymmetric padding when an icon is present (icon on left, text+gap on right);
        // symmetric padding when no icon — keeps text optically centered.
        ...
        return (
            ...
            >
                <div className="relative rounded-full overflow-hidden shadow-[0_20px_50px_-12px_rgba(0,0,0,0.25)]" style={{ padding: borderWidth }}>
                    ...
                    />
                    <div className={cn(
                        "relative z-10 rounded-full flex items-center bg-ink transition-colors group-hover:bg-neutral-900",
                        sizeStyles[size]
                    )}>
                        {icon && (
                            <div className={cn("rounded-full flex items-center justify-center bg-neutral-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.4)]", iconSizes[size])}>
